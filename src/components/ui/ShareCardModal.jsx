...
function parseBullets(text, isDoneField = true) {
  ...
  return lines.map((line) => {
    ...
    if (
      line.startsWith("✓") ||
      line.toLowerCase().startsWith("done:") ||
      line.startsWith("[x]")
    ) {
      isWip = false;
      isDone = true;
      ...
      line.startsWith("→") ||
      line.toLowerCase().startsWith("wip:") ||
      line.startsWith("[wip]") ||
      line.startsWith("[ ]")
    ) {
      isWip = true;
...
export function ShareCardModal({ log, isOpen, onClose }) {
  ...
  const handleDownload = () => {
    ...
    setTimeout(() => {
      ...
      const finalHeight = wrapHeight + 16; // 8px padding top + 8px padding bottom
  ...
  const handleCopyText = () => {
    const dsaText = dsaItems
      .map((item) => `${item.isWip ? "→" : "✓"} ${item.text}`)
      .join("\n");
    const jsText = jsItems
      .map((item) => `${item.isWip ? "→" : "✓"} ${item.text}`)
      .join("\n");
    const kredText = kredDisplayItems
      .map((item) => `${item.isWip ? "→" : "✓"} ${item.text}`)
      .join("\n");
    const tomorrowText = tomorrowItems
      .map((item) => `• ${item.text}`)
      .join("\n");
        ...
        >
          ...
          <div className="w-full overflow-x-auto flex justify-center py-2 bg-[#070a13] rounded-xl border border-dashed border-gray-800">
            <div
              ref={cardRef}
              className="dt-root"
              style={{ width: "540px", flexShrink: 0 }}
            >
              <style>{`
                @import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;700;800&family=DM+Sans:wght@400;500&display=swap');
                .dt-root { font-family: 'DM Sans', sans-serif; padding: 8px 0; background: #070a13; }
                ...
                <div className="dt-grid">
                  ...
                  <div className="dt-cell flex flex-col">
                    ...
                    <div className="dt-cell-content">
                      {dsaItems.length === 0 ? (
                        <span style={{ color: "#5F5E5A", fontStyle: "italic" }}>
                          No DSA logged today
                        </span>
                      ) : (
                  ...
                  <div
                    className="dt-cell flex flex-col"
                    style={{ borderTop: "0.5px solid #1e2535" }}
                  >
                      ...
                      {tomorrowItems.length === 0 ? (
                        ...
                        tomorrowItems.map((item, idx) => {
                          const isSpecial =
                            item.text
                              .toLowerCase()
                              .includes("machine coding") ||
                            item.text.toLowerCase().includes("system design") ||
                            item.text.includes(":") ||
                            (tomorrowItems.length >= 3 &&
                              idx === tomorrowItems.length - 1);
                          return (
                            <div
                              key={idx}
                              style={{
                                color: isSpecial ? "#5F5E5A" : "#D3D1C7",
                                fontWeight: isSpecial ? 500 : "normal",
                              }}
                            >
                              {item.text}
                            </div>
